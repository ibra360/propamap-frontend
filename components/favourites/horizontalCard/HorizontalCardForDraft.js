...
const HorizontalCardForDraft = (props) => {
  console.log('Horizontal card for draft -->', props)
  return (
    <div className='cmp-horizontal-card'  >
      <Row gutter={[16, 8]}>
        ...
        <Col span={13}>
          <Title className='property-title' level={4} onClick={() => redirect(`/properties/${props._id}`)} >{props.title}</Title>
        ...
        <Col span={5}>
          <div className='hotel-rent-details'>
            <Title
              level={4}
              className={`rent${props.rental_time &&
            ...
            {!props.hideFavButton && <Button
              icon='heart'
              className='add-favourite'
              shape='circle'
              category='primary'
              onClick={() => { props.onremoveFav(props._id) }}
              color={props.color}
            />}
            {props.hideFavButton && <div>
              ...
              <IconButton aria-label="delete" onClick={() => props.deleteMyDraft(props._id)}>
